<?php
	...
	if(isset($busqueda) && $busqueda!=""){
		...
		$consulta_datos="SELECT * FROM categoria WHERE categoria_nombre LIKE '%$busqueda%' OR categoria_descripcion LIKE '%$busqueda%' ORDER BY categoria_nombre ASC LIMIT $inicio,$registros";

		$consulta_total="SELECT COUNT(categoria_id) FROM categoria WHERE categoria_nombre LIKE '%$busqueda%' OR categoria_descripcion LIKE '%$busqueda%'";
		...
		foreach($datos as $rows){
			$tabla.='
				<tr class="has-text-centered" >
                    ...
                    <td>'.substr($rows['categoria_descripcion'],0,50).'</td>
                    <td>
